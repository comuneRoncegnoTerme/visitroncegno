...
export default async function Home() {
  ...
  return (
    <main>
      ...
      <section className="places-section" id="luoghi"><div className="section-shell"><div className="section-heading-row"><div><p className="eyebrow dark">Luoghi da conoscere</p><h2>Tre paesaggi, un’unica destinazione</h2></div></div><div className="places-grid">{featuredPlaces.map((place) => { const placeImage = getDirectusAssetUrl(place.image) ?? "/images/places/fallback.jpg"; const subtitle = place.category?.name ?? place.map_label ?? "Luogo da scoprire"; return (<a className="place-card" href={`/luoghi/${place.slug}`} key={place.id}><div className="place-image" style={{ backgroundImage: `url('${placeImage}')` }} /><div className="place-card-footer"><div><small>{subtitle}</small><h3>{place.title}</h3></div><span className="round-arrow dark-arrow"><ArrowIcon /></span></div></a>); })}</div></div></section>
